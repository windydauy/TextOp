...
def joint_vel_rel_no_ankle(env: ManagerBasedRlEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
...
def robot_anchor_ori_w(env: ManagerBasedRlEnv, command_name: str) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    mat = matrix_from_quat(command.robot_anchor_quat_w)
    return mat[..., :2].reshape(mat.shape[0], -1)


def robot_anchor_lin_vel_w(env: ManagerBasedRlEnv, command_name: str) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)

    return command.robot_anchor_lin_vel_w.view(env.num_envs, -1)


def robot_anchor_ang_vel_w(env: ManagerBasedRlEnv, command_name: str) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
...
def robot_anchor_pos_w(env: ManagerBasedRlEnv, command_name: str) -> torch.Tensor:
    """Robot anchor body position in world frame (single timestep)."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    return command.robot_anchor_pos_w.view(env.num_envs, -1)


def robot_anchor_pos_w_future(env: ManagerBasedRlEnv, command_name: str) -> torch.Tensor:
