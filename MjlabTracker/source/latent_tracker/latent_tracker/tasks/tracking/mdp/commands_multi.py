...
def resolve_motion_joint_indexes(
    ...
    if motion_joint_names is None:
        return list(range(len(requested_joint_names)))
    name_to_index = {name: index for index, name in enumerate(_decode_npz_names(motion_joint_names))}
    missing = [name for name in requested_joint_names if name not in name_to_index]
    if missing:
        raise ValueError(f"Motion file {motion_file} is missing joint_names required by tracker: {missing}")
    return [name_to_index[name] for name in requested_joint_names]
...
class MotionLoader:
    ...
    def __init__(self,
        ...
        joint_indexes = list(joint_indexes) if joint_indexes is not None else slice(None)
        self.fps = data["fps"]
        self.joint_pos = torch.tensor(data["joint_pos"][:, joint_indexes],
                                      dtype=torch.float32,
                                      device=device)

        self.joint_vel = torch.tensor(data["joint_vel"][:, joint_indexes],
                                      dtype=torch.float32,
                                      device=device)
...
class MultiMotionLoader:
    ...
    def __init__(self,
        ...
        self._requested_joint_names = list(joint_names) if joint_names is not None else []
        self.device = device
        ...
        for motion_file in motion_files:
            ...
            if self._requested_joint_names:
                motion_joint_names = data["joint_names"] if "joint_names" in data.files else None
                motion_joint_indexes = resolve_motion_joint_indexes(
                    motion_joint_names=motion_joint_names,
                    requested_joint_names=self._requested_joint_names,
                    motion_file=motion_file,
                )
            else:
                motion_joint_indexes = slice(None)

            jp = torch.tensor(data["joint_pos"],
                              dtype=torch.float32,
                              device=device)[:, motion_joint_indexes]
...
class MotionCommandCfg(CommandTermCfg):
    """Configuration for the multi-motion reference command."""
